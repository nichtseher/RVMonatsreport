...
import { Clock, Play, Square, Trash2, Shield, Calendar, Info, Check, HelpCircle, FileSpreadsheet, Plus } from "lucide-react";
...
export default function ClockInWidget({
  ...
  const [preset, setPreset] = useState<"office" | "field" | "half" | "custom">("half");
  ...
  const [manualPreset, setManualPreset] = useState<"office" | "field" | "half" | "custom">("half");
  ...
  const [typedManualOfficeHours, setTypedManualOfficeHours] = useState<string>("");
  const [typedManualFieldHours, setTypedManualFieldHours] = useState<string>("");
  ...
  const handleOpenClockOutForm = () => {
    ...
    announceToAriaAndSpeech("Schicht beendet. Formular zum Einbuchen geöffnet.", true);
  };
  ...
  const handleSaveShift = (e: React.FormEvent) => {
    ...
    const finalOfficeHrs = typedOfficeHours !== "" ? (parseFloat(typedOfficeHours) || 0) : calculatedOfficeHrs;
    const finalFieldHrs = typedFieldHours !== "" ? (parseFloat(typedFieldHours) || 0) : calculatedFieldHrs;
    const finalDuration = Math.round((finalOfficeHrs + finalFieldHrs) * 100) / 100;
    ...
    const newLog: TimeLog = {
      ...
      officeRatio: finalDuration > 0 ? Math.round((finalOfficeHrs / finalDuration) * 100) / 100 : 0.5,
  ...
  const calculateManualDuration = (cin: string, cout: string, breaks: number) => {
  ...
  const getCalculatedManualShiftValues = () => {
    const netHours = calculateManualDuration(manualClockIn, manualClockOut, manualBreakMinutes);
  ...
  const handleSaveManualShift = (e: React.FormEvent) => {
    ...
    const finalOfficeHrs = typedManualOfficeHours !== "" ? (parseFloat(typedManualOfficeHours) || 0) : calculatedManualOfficeHrs;
    const finalFieldHrs = typedManualFieldHours !== "" ? (parseFloat(typedManualFieldHours) || 0) : calculatedManualFieldHrs;
    const finalDuration = Math.round((finalOfficeHrs + finalFieldHrs) * 100) / 100;
  ...
  return (
    <div className="p-4 rounded-2xl border bg-[var(--card-bg)] border-[var(--border-color)] shadow-xs space-y-4">
      ...
      {!clockInTime ? (
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 p-1.5">
          <div className="space-y-1 text-center sm:text-left">
            <span className="block text-xs font-bold text-[var(--text-color)]">Aktuell nicht eingestempelt</span>
            <span className="block text-[10px] text-[var(--text-muted)] leading-relaxed">
              Starten Sie Ihre Schicht. Die App läuft im Hintergrund weiter (auch wenn Sie den Browser schließen).
            </span>
          </div>
          <button
            type="button"
          ...
          {!isFormOpen ? (
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-3 rounded-xl bg-slate-500/5 border border-[var(--border-color)]">
              <div className="text-center md:text-left space-y-1">
                <span className="text-[10px] font-black uppercase text-[var(--text-muted)] tracking-wider">Laufende Arbeitszeit</span>
                <div className="text-3xl font-black font-mono tracking-tight text-[var(--text-color)]">{elapsed || "00:00:00"}</div>
                <span className="block text-[10px] text-[var(--text-muted)] font-semibold">
                  Eingestempelt seit {new Date(clockInTime).toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit" })} Uhr am {new Date(clockInTime).toLocaleDateString("de-DE")}
                </span>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSaveShift} className="p-4 rounded-xl border border-[var(--border-color)] bg-[var(--bg-color)] space-y-4 animate-slide-up">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  <label htmlFor="break-input" className="text-xs font-bold text-[var(--text-muted)] block">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      type="button"
                      onClick={() => setBreakMinutes(prev => Math.min(240, prev + 15))}
                      className="px-2.5 py-1.5 rounded bg-[var(--card-bg)] border border-[var(--border-color)] text-xs font-bold cursor-pointer hover:bg-[var(--border-color)] active:scale-90"
                ...
                <div className="space-y-1">
                  ...
                  <div className="grid grid-cols-2 gap-1" role="radiogroup" aria-label="Arbeitszeit Aufteilung">
                    {[
              ...
              {preset === "custom" && (
                <div className="p-3 rounded-lg border border-[var(--border-color)] bg-[var(--card-bg)] space-y-2 animate-slide-up">
                  <label htmlFor="custom-ratio-slider" className="text-[10px] font-black uppercase text-[var(--text-muted)] tracking-wider flex justify-between">
                    <span>Aufteilung: {customRatio}% Büro / {100 - customRatio}% Außendienst</span>
                  </label>
                  <input
              ...
              <div className="p-3 rounded-xl border border-[var(--border-color)] bg-[var(--card-bg)] space-y-3">
                ...
                <p className="text-[10px] text-[var(--text-muted)] font-medium leading-relaxed">
                  Tragen Sie die Stunden bei Bedarf direkt manuell ein (mit 2 Nachkommastellen):
                </p>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label htmlFor="typed-office-hours" className="text-[11px] font-bold text-[var(--text-muted)] block">
                      Stunden Büro (h):
                    </label>
                    <input
                  ...
                  <div className="space-y-1">
                    <label htmlFor="typed-field-hours" className="text-[11px] font-bold text-[var(--text-muted)] block">
                ...
                <div className="text-[11px] font-black text-slate-500 dark:text-slate-400 flex justify-between bg-slate-500/5 p-2 rounded-lg">
                  ...
                  <span className="font-mono text-emerald-600 dark:text-emerald-400">
                    {((typedOfficeHours !== "" || typedFieldHours !== "")
                      ? ((parseFloat(typedOfficeHours) || 0) + (parseFloat(typedFieldHours) || 0))
              ...
              <div className="space-y-1">
                <label htmlFor="shift-notes" className="text-xs font-bold text-[var(--text-muted)] block">
        ...
        <div className="pt-2 border-t border-[var(--border-color)] flex justify-center">
          <button
            ...
            onClick={() => {
              ...
              announceToAriaAndSpeech("Formular für manuelles Nachtragen geöffnet.");
            }}
      ...
      {/* Manual Entry Form */}
      {isManualOpen && (
        <form onSubmit={handleSaveManualShift} className="p-4 rounded-xl border-2 border-dashed border-[var(--accent)] bg-[var(--bg-color)] space-y-4 animate-slide-up">
          <div className="flex items-center justify-between">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {/* Datum */}
            <div className="space-y-1">
              <label htmlFor="manual-date" className="text-xs font-bold text-[var(--text-muted)] block">
                Datum:
              </label>
              <input
                id="manual-date"
            ...
            {/* Kommen */}
            <div className="space-y-1">
              <label htmlFor="manual-clock-in" className="text-xs font-bold text-[var(--text-muted)] block">
                Kommen:
              </label>
              <input
                id="manual-clock-in"
            ...
            {/* Gehen */}
            <div className="space-y-1">
              <label htmlFor="manual-clock-out" className="text-xs font-bold text-[var(--text-muted)] block">
                Gehen:
              </label>
              <input
                id="manual-clock-out"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {/* Break */}
            <div className="space-y-1">
              <label htmlFor="manual-break-input" className="text-xs font-bold text-[var(--text-muted)] block">
                Pause abziehen (Minuten):
              </label>
              <div className="flex items-center gap-1.5">
                <button
                  type="button"
                  onClick={() => setManualBreakMinutes(prev => Math.max(0, prev - 15))}
                  className="px-2.5 py-1.5 rounded bg-[var(--card-bg)] border border-[var(--border-color)] text-xs font-bold cursor-pointer hover:bg-[var(--border-color)] active:scale-90"
                  aria-label="Pause um 15 Minuten verringern"
                >
                  -15
                ...
                <input
                  ...
                  onChange={(e) => setManualBreakMinutes(Math.max(0, parseInt(e.target.value) || 0))}
                  className="flex-1 p-1.5 border border-[var(--border-color)] bg-[var(--input-bg)] text-[var(--text-color)] text-xs font-bold rounded text-center outline-none"
                />
                <button
                  type="button"
                  onClick={() => setManualBreakMinutes(prev => Math.min(240, prev + 15))}
                  className="px-2.5 py-1.5 rounded bg-[var(--card-bg)] border border-[var(--border-color)] text-xs font-bold cursor-pointer hover:bg-[var(--border-color)] active:scale-90"
                  aria-label="Pause um 15 Minuten erhöhen"
                >
                  +15
            ...
            <div className="space-y-1">
              ...
              <div className="grid grid-cols-2 gap-1" role="radiogroup" aria-label="Manuelle Arbeitszeit Aufteilung">
                {[
                  { id: "half", label: "⚖️ 50% / 50% " },
                  { id: "office", label: "💻 100% Büro" },
                  { id: "field", label: "🚗 100% Außen" },
          ...
          {manualPreset === "custom" && (
            <div className="p-3 rounded-lg border border-[var(--border-color)] bg-[var(--card-bg)] space-y-2 animate-slide-up">
              <label htmlFor="manual-custom-ratio-slider" className="text-[10px] font-black uppercase text-[var(--text-muted)] tracking-wider flex justify-between">
                <span>Aufteilung: {manualCustomRatio}% Büro / {100 - manualCustomRatio}% Außendienst</span>
              </label>
              <input
          ...
          <div className="p-3 rounded-xl border border-[var(--border-color)] bg-[var(--card-bg)] space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label htmlFor="typed-manual-office-hours" className="text-[11px] font-bold text-[var(--text-muted)] block">
                  Stunden Büro (h):
                </label>
                <input
              ...
              <div className="space-y-1">
                <label htmlFor="typed-manual-field-hours" className="text-[11px] font-bold text-[var(--text-muted)] block">
                  Stunden Außendienst (h):
                </label>
                <input
            ...
            <div className="text-[11px] font-black text-slate-500 dark:text-slate-400 flex justify-between bg-slate-500/5 p-2 rounded-lg">
              ...
              <span className="font-mono text-emerald-600 dark:text-emerald-400">
                {((typedManualOfficeHours !== "" || typedManualFieldHours !== "")
                  ? ((parseFloat(typedManualOfficeHours) || 0) + (parseFloat(typedManualFieldHours) || 0))
          ...
          {/* Notes */}
          <div className="space-y-1">
            <label htmlFor="manual-shift-notes" className="text-xs font-bold text-[var(--text-muted)] block">
              Kurzkommentar / besuchte Schule / Ort (optional):
            </label>
            <input
        ...
        <div className="pt-2 border-t border-[var(--border-color)]">
          <button
            ...
            onClick={() => {
              ...
              announceToAriaAndSpeech(isLogsCollapsed ? "Schichtprotokoll ausgeklappt" : "Schichtprotokoll eingeklappt");
            }}
          ...
          {!isLogsCollapsed && (
            <div className="space-y-3 mt-2.5">
              ...
              {timeLogs.map((log) => {
                ...
                return (
                  <div key={log.id} className="py-2 flex items-center justify-between gap-2.5 text-xs">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        <span className="text-[10px] text-[var(--text-muted)] font-medium">
                          ({log.clockIn} - {log.clockOut}, Pause {log.breakMinutes}m)
                        </span>
                      </div>
                      ...
                      {log.notes && (
                        <p className="text-[10px] text-[var(--text-muted)] italic font-medium mt-0.5 truncate" title={log.notes}>
                          "{log.notes}"
                        </p>
                      )}
                    </div>
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        if (confirm(`Schicht vom ${formattedDate} (${log.duration.toFixed(2)}h) wirklich löschen? Die Stunden werden automatisch von der Monatsübersicht abgezogen.`)) {
                          onDeleteLog(log);
                        }
                      }}
