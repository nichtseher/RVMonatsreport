...
| **2.4.3 Fokus-Reihenfolge** (0.9.41) | Nach einem Ansichtswechsel stand der Fokus nirgends Sinnvollem: bei den fünf Hauptansichten blieb er auf der Navigationstaste — die im Dokument **hinter** dem Inhalt steht, die neue Ansicht war vorwärts also gar nicht erreichbar —, bei zwei Ansichten auf `document.body`, bei fünf auf deren Zurück-Taste. Jetzt auf der Überschrift der neuen Ansicht, in allen zwölf. Die Wiederherstellung des vorigen Fokus, die fünf Ansichten mitführten, war nachweislich wirkungslos (das gemerkte Element hängt beim Schließen nicht mehr im Dokument). |
| **4.1.2 Name, Rolle, Wert** (0.9.41) | Die untere Navigationsleiste trug `role="tablist"`/`role="tab"`/`aria-selected` ohne `role="tabpanel"`, ohne Pfeiltastenbedienung und ohne gemeinsamen Tabulatorhalt — eine Rolle, die eine Bedienung verspricht, die es nicht gab. Dieselbe Navigation trat am Schreibtisch als `<nav>` ganz ohne Markierung des aktuellen Eintrags auf. Beides jetzt Navigation mit `aria-current="page"`. **axe-core meldet das nicht**: Ein `tablist` mit `tab`-Kindern ist strukturell vollständig. |
