...
npm run check      # tsx scripts/pruefen.ts — 198 checks, no test framework
...
`npm run check` covers the pure functions where a mistake does real damage: sync merge (including the per-field timestamps), Excel sum formulas, working-time math across midnight, backup encryption, and a scan of the whole project for double-encoded characters and BOMs (only `src/` until 0.9.33 — the one real find sat in `vite.config.ts`, right next to it). Add cases there rather than writing another throwaway script. `scripts/checks/kodierung.ts` has **no allowlist any more**: the one deliberate mojibake lived in the 0.7.0 changelog entry, and that entry became one line with the shorter changelog policy in 0.9.40. An exception without a case is an open door.
...
**A hand-maintained list of states is not a safety net — it has been forgotten three times.** `EINSTIEGE` missed `ManageModal` entirely (0.9.18–0.9.21, three versions measuring the wrong view), `ZUSTAENDE_MIT_SCHRIFT` missed the first-run wizard in the very version that introduced the list and told the reader to keep it current, and `RUECKFRAGEN` missed the seventh confirmation on 2026-09-12 while its own DEVLOG entry claimed "all six". Every time the run stayed green, because a gate measures what its list names and cannot notice an omission. The answer is a counting check that compares source against list — `scripts/checks/zustandsdeckung.ts` counts `useState` switches, `scripts/checks/rueckfrage.ts` counts `setConfirmRequest` call sites against `RUECKFRAGEN` (ten since 0.9.33; it reported the three missing entries itself on the first run). **Add one whenever you add a list.**

**And a view is not the smallest thing that goes unchecked — a state inside it is.** Eight times now the defect has sat one level below what `?tab=` or a menu click reaches: `manage`, the time-clock forms, the sync states, the archive with content, the form states, the first-run wizard, the shift log, and — in 0.9.32 — the confirmations. The last one is the sharpest example of the pattern, because 0.9.22 had already written down that no check ever opened a `ConfirmDialog`, and it stayed that way for ten versions. When you add a state that a `useState` switch or a click produces, add it to the matching `*_ZUSTAENDE` list in `tests/oberflaeche.spec.ts`; `scripts/checks/zustandsdeckung.ts` counts the switches so a new one cannot pass unnoticed.

**The app has thirteen views, not five.** `activeTab` takes thirteen values (eleven until 0.9.36 added `bestand`, twelve until 0.9.47 added `erklaerung`); only six are reachable through `?tab=` (the switch in `App.tsx` accepts exactly those). The other seven — `manage`, `help`, `sync`, `backup`, `carryover`, `changelog`, `erklaerung` — have to be clicked, and that is precisely why they went unchecked until 2026-09-02. `ManageModal`, `HistoryModal`, `StatsModal` and `TimeModal` are **not dialogs** despite their names; they are full views. Covering the six behind their entry points found six real defects on the first run, four of which hit the target group directly (keyboard-unreachable scroll region, two reflow failures at "Extra groß", a shrunken back button). When you add a view, add it to `EINSTIEGE` in `tests/oberflaeche.spec.ts` — nothing else will notice it exists.

**And an entry in `EINSTIEGE` is not proof that the view is covered.** From 0.9.18 to 0.9.21 this file claimed all eleven were checked; ten were. The entry named "Formular anpassen" clicks a menu row in `A11yModal` and lands on a **submenu of the same name** — not a separate `activeTab` at all. `ManageModal` sits one level below it, behind "Eigene Felder löschen", and is headed "Formularfelder verwalten" (since 0.9.68: "Eigene Felder löschen" — every view heading now equals the menu entry that leads to it, via `AnsichtsKopf`). The test waited for the heading "Formular anpassen", found it immediately, and measured the wrong view for three versions. **Assert on a heading that only the intended view has**, and when a view needs two clicks, use the `dann` field. The first real run against `manage` produced six failures and five defects, including the keyboard trap that had been removed from `CarryoverModal` a week earlier.
...
**But "~33s" is from before `check:ui` joined the gate — budget ~23 minutes.** Measured 2026-09-17: `e74c579` ran 20:28:25 → 20:51:57, `bd81f97` took about the same. The gate runs `lint`, `check`, **`check:ui`** and `audit` before building, and the UI run alone is ~20 minutes. Waiting 8 minutes and then finding the old bundle still live looks exactly like the lost deploy of 2026-08-08 and is nothing of the sort — that false alarm happened here. Poll the run's `status`/`conclusion` (authenticated, so the 60/hour cap does not bite), and only look at the served bundle once the run reports `completed success`.

**But a successful push does not guarantee a run was created.** On 2026-08-08 the push of `cc5f2b0` (0.9.8) reported `7aade11..cc5f2b0 main -> main` and no workflow run for that SHA ever appeared — checked against the last 12 runs, the list jumps straight from `7aade11` (0.9.7, 2026-08-03) to `63b11a9` (0.9.9, minutes after the 0.9.8 push). Cause unknown; Actions worked again for the very next push. 0.9.8 only reached production because 0.9.9 is a descendant and carried it along. **Confirm the run exists for the SHA you pushed** rather than assuming the push implies a deploy.

**Since 0.9.35 a failed deploy files its own report.** The workflow has an `if: failure()` step that opens a GitHub issue (or comments on the open one labelled `deploy-fehler`), because GitHub notifies repo watchers about issues on its own — the account-level workflow-notification setting is outside the repo and cannot be set from here. **The trigger is verified since 2026-09-15**: the failed deploy of `3101833` (0.9.45) opened issue #1 "Deploy fehlgeschlagen", labelled `deploy-fehler`, naming the SHA and the run URL. **What follows from that is a duty, not a comfort:** the workflow comments on the *open* issue carrying that label instead of opening a new one, so an issue left open turns the next failure into a comment under a headline everyone has already read. Close it once the deploy that follows is green — and check that it is, rather than assuming.

**And "it exists" is not "it worked" — check the conclusion.** On 2026-09-07 the deployment history showed that `f8ae712` and `e638f68` (both 2026-09-02) had runs, had deployment records, and carried the state `failure`: production had been sitting on `497fef3` for five days and nobody had noticed. Both died on the same step, `npm run check:ui`, with the same message — `Formular: per Tabulator nicht erreichbar — BUTTON"RV Archiv" | BUTTON"Optionen"` — a timing race that the loaded runner exposes and this machine does not. The DEVLOG entry for that commit reports a green local run, and it *was* green locally. **A green local gate is not a green deploy gate.** Two calls settle it; the second is the one that actually answers "what is live":

```
/actions/runs?per_page=5     -> status + conclusion per SHA
/deployments?per_page=4      -> then each statuses_url -> state (success | failure)
```

Job logs need authentication (403 without). The push credential works as a bearer token: `printf 'protocol=https\nhost=github.com\n\n' | git credential fill` yields it — never echo it, and read the log via `/actions/jobs/<id>/logs`. `npm run deploy` **is gone** (removed in 0.9.20, along with the `gh-pages` devDependency). It published to a `gh-pages` branch that has determined nothing since 0.9.2, and it read like the real deploy path — the ROADMAP called it a trap for anyone who believed it. The only thing that publishes is a push to `main`.
...
- **Refactors that must change nothing: diff the rendered DOM, before and after.** A refactor has no new state to look at, so every difference is a defect — and `tsc` sees types, `check` sees pure functions, `check:ui` sees geometry and axe. None of them would notice a dropped `aria-live` or a hint that stopped rendering. Take `document.getElementById("root").outerHTML` (one element per line, `replace(/></g, ">\n<")`) in a handful of states and compare. 0.9.42 did this across five states — form at 360, form at 360 **with the time clock switched off**, options, form at 1280 (sidebar instead of bottom bar), archive at 1280 — and got 0 differences three times running. Include a state that exercises each conditional you are touching: the "clock off" state is what would have caught the `.filter(tab => tab.visible)` that the rewrite script silently ate. Expect the wall clock in "Automatisch lokal gesichert (hh:mm:ss)" to differ; that is the one known false positive.
...
- **A `page.addInitScript` written in a standalone `tsx` script dies with `__name is not defined` — and dies silently.** esbuild (which `tsx` uses) wraps named functions in a `__name()` helper that does not exist in the browser, so the whole init script aborts at the first such function. On 2026-09-15 that made three rounds of "reproduced with the forced wide font" measure plain Segoe UI, and the deploy failed on exactly the case those measurements had cleared. Playwright's own runner compiles spec files differently and is **not** affected — verified with a throwaway spec that printed the effective font (`Verdana, "DejaVu Sans"`, one style element, no page errors), so `erzwingeBreiteSchrift` in the gate does work. Two rules follow: read `page.on("pageerror")` when an injected script seems to do nothing, and prefer a throwaway **spec file** over a standalone script whenever the measurement must match what the gate sees.
...
**A dev server that has been running across your edits can serve a stale module — and then you measure the old code.** On 2026-09-07 five class strings were replaced in `DeviceSyncModal.tsx`, `tsc` was clean, the old strings were gone from the file and present in no other file — and the browser still rendered them. Three rounds of "the fix has no effect" went into that before the cause was obvious. The counter-check is one line: read the live `className` in the page and compare it with the file. If they differ, kill the server on port 3000 and let Playwright start a fresh one. This is the mirror image of the "don't edit during a run" rule above: the run itself is not the only thing that goes wrong.

**Line endings are a property of the last write, not of the file — measured 2026-09-17.** `core.autocrlf=true` and there is no `.gitattributes`, so whatever git checks out (or restores from a stash) lands in the working tree as **CRLF**, while every Write/Edit and every `writeFileSync` from a script lands as **LF**. The same file measured 0 of 3447 lines with `\r` right after an editing round and 3467 of 3467 after a single `git stash push` / `pop`. So a multi-line anchor built with `\n` works or fails depending on what touched the file last, and the failure looks exactly like a wrong anchor — three attempts went into that on 2026-09-07 before the cause was obvious. Never assume either: detect the ending (`s.includes('\r\n')`) and build anchors from it, or keep anchors single-line, where the difference cannot bite. An edit can also leave a file **mixed**, which is why a grep for `\r` counts lines rather than answering yes or no.
...
**A Git-Bash path is not a path for Windows Python — and the failure reads as something else entirely.** `python -c "...open(r'/c/Users/...')"` from the Bash tool dies on a path that `ls` in the same shell resolves fine: MSYS rewrites `/c/...` for its own binaries, native Python never sees a drive. On 2026-09-17 a deploy watcher built that way reported *"API answer unusable"* for a deploy that was running normally. Pipe the data in on **stdin** (`curl … | python -c "json.load(sys.stdin)"`) or hand Python a `C:\...` path; and when a watcher reports an infrastructure problem, check its own plumbing before believing it.
...
**Single monolithic state container.** `src/App.tsx` (~3200 lines since 0.9.42) owns essentially all application state (report data, history/archive, accessibility settings, goals, carryover, quick-entry config, sync handlers, dictation, TTS) via `useState`/`useCallback`, and passes state + handlers down as props to view components in `src/components/`. There is no router — navigation is a single `activeTab` string switched by the bottom nav / sidebar, with each tab's component rendered conditionally at the bottom of `App.tsx`'s JSX. When adding a feature, the handler almost always belongs in `App.tsx` and is threaded down via props, following the existing `handleXxx` naming convention.

Two lists inside it are the shape everything repetitive should take: `bereiche` (the four report sections, rendered through one `BerichtsBereich`) and `hauptnavigation` (the five main views, rendered by *both* nav bars). Before 0.9.42 each of those existed as near-identical JSX — four blocks and two bars — and the navigation had already drifted apart between mobile and desktop (see 0.9.41). Put the differences in the data (`hinweis`/`fuss`, `ansage`), not in a branch inside the component, and never write the second copy.
...
**Wiping the device goes through `loescheAllesLokal()` (`src/utils/allesLoeschen.ts`)** — used by "Alle Daten von diesem Gerät löschen" in the options (0.9.43) and by the crash screen. It clears **IndexedDB first, then `localStorage`**, and that order is the reason the failure path can be honest: IndexedDB is the half that can fail (locked database, private mode), and if it does, nothing is gone yet. The other way round the app would sit there without settings but with a full archive, and every message about it would be wrong.
...
- **Both Excel outputs** live in `src/utils/vorlageExport.ts` — the monthly report (sheet 1 *is* the company template, filled with ExcelJS since 0.9.11) and the separate timesheet (moved there in 0.9.33, when SheetJS was removed: the same shift table existed twice, in two libraries, 1.44 MB). `src/utils/excelUtils.ts` now holds only `triggerFileDownload`, which returns `"geteilt" | "heruntergeladen" | "abgebrochen"` so each caller can report the outcome honestly (a cancelled share is not an error and must not trigger a download). The shift table itself is one function, `baueZeitenBlatt`, used by sheet 3 and by the standalone file.
- **What leaves the device is a choice, not a default (0.9.33).** `erzeugeVorlagenDatei(data, appFields, umfang, mitZeitenblatt)` takes `umfang: "vorlage" | "alle"` **without a default value** — on purpose: a silent default that ships everything is exactly the failure this switch exists to prevent, so the compiler makes every caller decide. `"vorlage"` is sheet 1 alone; `"alle"` adds the two RV-Mobil sheets. Both export paths (form and archive) ask before every send, and "Nur Vorlage senden" is the proposed answer. The reason is not cosmetic: sheet 3 carries per-shift clock-in/clock-out/break/notes, and the company template itself only ever asks for working days (D18) and office hours (D19). That is the § 87 BetrVG surface, and it is now user-controlled — see ROADMAP open question 4.
- **The embedded template's version lives in `src/utils/vorlageStand.ts`, in its own file, and that is not tidiness.** Two measured reasons (2026-09-14): putting it next to the template in `vorlageMonatsinfo.ts` dragged the 16 KB base64 blob into the **start bundle** (581,777 → 598,106 bytes) and pulled it *out* of the export chunk, so everyone would load it at startup instead of only on export; and putting it in `version.ts` broke `npm run check`, because that file reads `__APP_VERSION__`, which Vite only supplies at build time while the checks run under `tsx`. Grep the built start bundle for `UEsDB` (the PK signature of an .xlsx) to catch the first trap.
- **`VORLAGE_STAND` must appear in every generated file.** Until 0.9.35 the template's provenance existed only in a source comment, which hit the product's core promise: if the company issues a new form, the app keeps filling the old one and the file still *looks* like their form. It now goes into the workbook's document properties — the only place that exists regardless of `umfang`, and the "nur Vorlage" path is exactly the one the Vertriebsleitung gets — plus visibly on sheet 2, the pre-send confirmation and the help. **Sheet 1 stays untouched**; a note in an unplanned cell would break the promise it is meant to protect. `scripts/checks/vorlage.ts` asserts all of it. Whoever swaps the template changes this constant with it.
...
**Lazy-loaded screens: eleven of the thirteen views** (all but the form view and `A11yModal`, the options hub; ten of twelve since 0.9.43, plus `erklaerung` in 0.9.47). Start bundle 563.67 → **382.26 KB** (145.77 → 111.34 gzipped). Keep new screens lazy too — but never without the second half of the deal: **a lazy chunk is not in `index.html`, so `sw.js` does not precache it**, and a colleague who updates and then drives into a dead spot would find that view missing. `App.tsx` therefore prefetches every lazy view in the background (`requestIdleCallback`, plain timer on Safari — which is what the colleagues actually run), and the service worker's fetch handler caches each one. Measured 2026-09-15 against the built app: all eight new chunks fetched within six seconds of an idle start. If you add a lazy view, add it to that prefetch list.
...
- Modal dialogs need a real focus trap, not just `aria-modal="true"`; browsers do not enforce containment. **`OnboardingModal.tsx` is the reference implementation, not `ConfirmDialog.tsx`** — that was wrong here until 0.9.32. A trap that only compares `activeElement` against the first and last focusable element does nothing whenever focus is somewhere else entirely, and "somewhere else" includes the background. `OnboardingModal` has the branch that pulls an outside focus back in; `ConfirmDialog` did not, and that is exactly how the keyboard came to walk through the page *behind* a delete confirmation (measured 2026-09-12, two of five confirmations). Both have it now. The `tabindex="-1"` heading is one way to land in that state, not the only one.
- **A focus effect must not depend on a prop that is a fresh arrow every render.** Every `onClose` in `App.tsx` is inline (`onClose={() => setActiveTab("options")}`), so `useEffect(..., [isOpen, onClose])` re-ran on every App render — cleanup yanked focus back to the trigger, setup moved it again 50 ms later. Four of eleven views threw the keyboard focus back to their close button on *any* App re-render; an announcement was enough, and this app announces almost everything. Keep `onClose` in a ref and depend on `[isOpen]` alone. `useCallback` at the call site would fix the five known cases and leave the trap set for the sixth.
- **A view change moves focus to the new view's heading — centrally, in `useAnsichtsFokus`, not per view.** Every view marks its top heading with `data-ansicht-titel` + `tabIndex={-1}`; `scripts/checks/ansichtsfokus.ts` counts the `activeTab` values against that list, so a new view cannot ship without one. Three things that were measured on 2026-09-15 and are easy to get wrong again: a fixed `setTimeout` is not enough (the sync and backup views are `React.lazy` — the placeholder is what sits in the DOM 50 ms later, so the hook searches by `requestAnimationFrame` until the heading exists); the skip on page load must compare the **previous `activeTab`**, never a "has run before" flag, because `StrictMode` runs effects twice in dev and consumes the flag — that failure would have existed *only* in development; and a focus trap that only compares against first/last focusable lets Shift+Tab escape from a `tabindex="-1"` heading (`DeviceSyncModal`, the one real dialog, needed the `OnboardingModal` containment branch).
- **The bottom navigation has four stations and short labels, and both are measured, not styled.** Until 0.9.44 it had five (`RV Report · RV Zeit · RV Analyse · RV Archiv · Optionen`) and at "Extra groß" **every one of the five labels was clipped** — 51 px per button at 360 px, 44 px at 320 px, against 57–88 px needed. Nothing caught it in 1,086 checks: `truncate` overflows nothing (the overflow guard stays quiet), the accessible name stays complete (axe stays quiet), and the buttons themselves are big enough (the target-size guard stays quiet). The people it hurt were the ones who set the large font — sighted low-vision users, for whom five buttons reading "RV A…", "RV R…", "RV Ar…" differ only by icon. At 320 px the bar has 292 px inside: 49 px per button with five, 62 px with four. `Report` 53, `Analyse` 61, `Archiv` 49, `Zeit` 30, `Mehr` 42 — four fit, five never will. `tests/oberflaeche.spec.ts` now fails on any clipped label and on a fifth station; if you need one, take one away.
- **The content area is a real `<main>` (0.9.47), and `banner`/`contentinfo` do not belong inside it.** Until 0.9.46 `#main-content` was a `<div>` with an id: the skip link worked, the landmark did not exist, so the second route to the content — NVDA `D`, the VoiceOver rotor's "Landmarks" — found nothing. That was the gap the conformance report listed under 1.3.1. The form's header card carried `role="banner"` and the footer `role="contentinfo"`; both are *page* landmarks and are misplaced inside `main`, so they went with the change (the `<header>`/`<footer>` elements stay). The mobile bottom bar still sits inside `main` while the desktop sidebar sits outside it — allowed, asymmetric, and written down rather than silently fixed.
- **The bottom navigation is a navigation, not a tablist.** It claimed `role="tablist"`/`role="tab"`/`aria-selected` until 0.9.40 with no `role="tabpanel"`, no arrow keys and no roving tabindex — a promise the keyboard never kept, and one axe cannot see (a tablist with tab children is structurally complete). It is `aria-current="page"` now, in both the mobile bar and the desktop sidebar, and `ansichtsfokus.ts` rejects a `role="tab"` that comes back without a `tabpanel`.
...
- **`ChangelogModal.tsx`** — user-facing, plain language, every version tagged Beta. **Since 0.9.40 the depth depends on the release** (owner's decision): spell a version out only when the user can newly *do* something, or must newly *know* something in order to act — a new view, a new choice, a disclosure. Everything else gets a category and one line: `Fehlerbehebungen` / `Verbesserungen` / `Wartung`. Renames, layout, contrast, internal checks and security work that changes nothing visible are all "everything else".

  Two consequences that are easy to miss when writing a new entry: a collapsed entry can orphan a later one that referred to it (0.9.37 announced a rename and became `Fehlerbehebungen`, so the 0.9.36 entry was rewritten to carry the *current* name — a reader searches for what is on their screen), and the judgement call belongs in the changelog file itself, not in a heuristic. 0.9.38 stays spelled out although it is not a feature: it discloses that dictation sends audio to Google/Apple, and burying that would defeat the point of having written it.
- **`BARRIEREFREIHEITSERKLAERUNG.md` and `BarrierefreiheitModal.tsx` carry the same statement twice, on purpose** (0.9.47): the repo needs a file, the colleagues need a view (*Optionen → Erklärung zur Barrierefreiheit*). Duplication is a defect everywhere else in this project — what makes it tolerable here is `scripts/checks/erklaerung.ts`, which compares the load-bearing claims (the legal classification, the "teilweise vereinbar" status, the contact, the three outstanding proofs) and rejects leftover placeholders, while deliberately not comparing wording. **The classification is the hinge:** RV Mobil is an employee work tool, so neither BFSG nor BITV 2.0 applies and §§ 164 SGB IX / 3a ArbStättV do; the statement exists anyway, by the owner's decision. If the app is ever opened to people outside the company, that paragraph is the first thing that has to change.
...
- **Der häufigste Layoutfehler dieses Projekts, mit Abstand: `min-width: auto` an Flex-Elementen.** Ein Flex-Kind gibt seine Breite standardmäßig **nicht** unter seinen Inhalt preis. Bei großer Schrift oder einer breiteren Schriftart sprengt es dann die Zeile, statt umzubrechen — die Seite wird waagerecht scrollbar (WCAG 1.4.10). Am 2026-09-02 war das **siebenmal** die Ursache: Überschrift im Jahreskonto, dessen Tastenzeile, die Versionsüberschriften im Changelog, die Zurück-Taste dort, die Reiter der Zeit-Ansicht, der Umschalter der Analyse und die Taste „Was gibt's Neues?" in den Optionen.
  ...
  Vier Gegenmittel, je nach Fall: `min-w-0` am Kind, wenn der Text umbrechen darf; `flex-wrap` am Container, wenn ganze Elemente untereinander sollen; `[overflow-wrap:anywhere]`, wenn ein einzelnes langes deutsches Wort nicht passt — `break-words` genügt dafür **nicht**, es ändert die intrinsische Mindestbreite nicht; und **Stapeln** (`flex-col` bis `sm:flex-row`), wenn keins davon greift.
...
- **Keine Versalien, keine Sperrung, eine kleine Schriftstufe** (Entscheidung
  vom 2026-09-17, 0.9.47). `uppercase`, `lowercase`, `capitalize`, jedes
  `tracking-*` und `text-[0.6875rem]` sind in Klassenlisten unzulässig;
  `scripts/checks/typografie.ts` weist sie mit Dateinamen ab. Großbuchstaben
  nehmen dem Wort die Umrissform, an der geübte Leser es erkennen — für blinde
  Nutzer folgenlos (`text-transform` ändert den Text nicht), für die
  sehbehinderte Hälfte der Zielgruppe nicht. Die Sperrung war deren
  Gegenmittel und ist ohne sie nur noch schlechter lesbar. Gemessen vorher/
  nachher über sechs Ansichten und 182 Elemente: Versalien 19 → **0**,
  gesperrte Elemente 20 → **0**, Schriftgrößen 8 → **7**.

  Zwei Fallen, beide am 2026-09-17 zugeschnappt: Ein solcher Wächter darf
  **nur in Klassenlisten suchen**, sonst meldet er den Kommentar als Verstoß,
  der erklärt, warum die Klasse entfallen ist — und wer den Verstoß
  wegräumen will, löscht die Begründung. Und `lowercase` stand an einer
  Stelle **als Gegengift** zum `uppercase` der Elternzeile: Ohne sie schrieb
  es ein deutsches Substantiv klein. Nach einem solchen Umbau noch einmal im
  Browser messen, nicht im Quelltext.
