...
npm run check      # tsx scripts/pruefen.ts — 191 checks, no test framework
...
**Lazy-loaded screens: ten of the twelve views since 0.9.43** (all but the form view and `A11yModal`, the options hub). Start bundle 563.67 → **382.26 KB** (145.77 → 111.34 gzipped). Keep new screens lazy too — but never without the second half of the deal: **a lazy chunk is not in `index.html`, so `sw.js` does not precache it**, and a colleague who updates and then drives into a dead spot would find that view missing. `App.tsx` therefore prefetches every lazy view in the background (`requestIdleCallback`, plain timer on Safari — which is what the colleagues actually run), and the service worker's fetch handler caches each one. Measured 2026-09-15 against the built app: all eight new chunks fetched within six seconds of an idle start. If you add a lazy view, add it to that prefetch list.
