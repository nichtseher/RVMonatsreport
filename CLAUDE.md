...
**Since 0.9.35 a failed deploy files its own report.** The workflow has an `if: failure()` step that opens a GitHub issue (or comments on the open one labelled `deploy-fehler`), because GitHub notifies repo watchers about issues on its own — the account-level workflow-notification setting is outside the repo and cannot be set from here. **The trigger itself is still unverified**: it only fires on a real failure. What is verified is the YAML structure, the embedded script's JS syntax (`node --check` against the extracted block) and the three Octokit method names. Do not treat a quiet Actions tab as proof it works.
